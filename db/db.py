...
def getOntologyTermTree(id='0',dn=None):
    ...
    ### Unfortunately, it is necessary to retrieve the entire ontology
    ### table to construct even partial trees because the order is
    ### unknown perhaps some research on tree representations in SQL
    ### would help
    ...
    ### Create tree structure for each head of the ontology may be
    ### multiple trees, they have parent as None we will place them
    ### under 'root' node if the whole tree is requested
    ot_tree=t.Tree()
    ot_tree.create_node('root','0')

    # make sure parents always occur before children try to insert, if
    # parent is not in tree, skip repeat until all records are
    # inserted
    while len(records)>0:
        for o in records:
            pid=o['parent_uid']
