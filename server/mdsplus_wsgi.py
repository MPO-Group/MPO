...
class application:
    ...
    def __init__(self, environ, start_response):
        #at least as of uwsgi 1.9, environ doesn't get user environment
        script_filename=environ.get('SCRIPT_FILENAME')
        if not script_filename and os.environ.has_key('SCRIPT_FILENAME'):
               script_filename=os.environ['SCRIPT_FILENAME']
        else:
               print("Warning from mdsplus_wsgi.py, SCRIPT_FILENAME env var not found.")
...
#ugly patch for lack of redirect
        self.path_parts=self.environ['PATH_INFO'].split('/')[2:] 
        print('mdsplus debug '+self.environ['PATH_INFO'])
    ...
    def __iter__(self):
	  ...
	  self.start('500 BAD_REQUEST',[('Content-type','text/text')])
          ...
          print('mdsplus_wsgi.py '+str(response_headers))
        print('got here')
        #better would be to add web server here instead of '*'
        response_headers.append( ('Access-Control-Allow-Origin', '*') )
        print(status,response_headers,output)
